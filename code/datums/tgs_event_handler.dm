...
/datum/tgs_event_handler/impl/HandleEvent(event_code, ...)
	switch(event_code)
		...
		if(TGS_EVENT_DEPLOYMENT_COMPLETE)
			...
			to_chat(world, "<span class='tgsmsg'>Server updated, changes will be applied on the next round...</span>")
		...
		if(TGS_EVENT_WATCHDOG_REATTACH)
			...
			if(!old_version.Equals(new_version))
				to_chat(world, "<span class='tgsmsg'>TGS updated to v[new_version.deprefixed_parameter]</span>")
			else
