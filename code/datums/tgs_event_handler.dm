...
/datum/tgs_event_handler/impl/HandleEvent(event_code, ...)
	switch(event_code)
		...
		if(TGS_EVENT_DEPLOYMENT_COMPLETE)
			...
			to_chat(world, "<div class='tgsmsg'>Сервер обновился, изменения будут применены в следующем раунде...</div>")
		...
		if(TGS_EVENT_WATCHDOG_REATTACH)
			...
			if(!old_version.Equals(new_version))
				to_chat(world, "<div class='tgsmsg'>TGS updated to v[new_version.deprefixed_parameter]</div>")
			else
